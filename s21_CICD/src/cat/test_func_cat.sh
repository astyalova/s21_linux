...
"-n test_files/test_1_cat.txt test_2_cat.txt"
...
testing()
{
    ...
    ./s21_cat $t > test_1_cat.log
    cat $t > test_sys_cat.log
    DIFF_RES="$(diff -s test_1_cat.log test_sys_cat.log)"
    if [ "$DIFF_RES" == "Files test_1_cat.log and test_sys_cat.log are identical" ]
    then
      (( SUCCESS++ ))
    ...
    rm test_1_cat.log test_sys_cat.log
}
